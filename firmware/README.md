...
It reads WAV files from an SD card, applies the preprocessing required by the
compiled model, runs inference on the dedicated NPU, and reports detections over
UART.
...
> RTOS. This makes it easy to validate the complete input-to-classification
...
│    ┌──────────┐   ┌─────────────────┐   ┌──────────────┐         │
│    │wav_reader│──►│ frontend branch │──►│ NPU runtime  │──► UART│
│    │  (.c/.h) │   │ raw/STFT/mel    │   │ (LL_ATON)    │         │
│    └──────────┘   └─────────────────┘   └──────────────┘         │
│         ▲          ▲                       ▲                      │
│    sd_handler   audio_stft/audio_mel  network.c (generated)      │
│    FatFs+BSP_SD CMSIS-DSP (FFT)       stedgeai output            │
└─────────────────────────────────────────────────────────────────┘
```
...
| **STFT** | `audio_stft.c/h` | Hann-windowed magnitude STFT |
| **FFT** | `Drivers/CMSIS-DSP/` | CMSIS-DSP real FFT and complex magnitude, Helium-vectorized (vendored subset) |
| **Mel filter** | `audio_mel.c/h` | Librosa-compatible mel projection for precomputed models |
| **SD handler** | `sd_handler.c/h` | BSP_SD init, FatFs mount, and directory scan |
...
### Step 2: Frontend Preprocessing

The branch is selected at build time from the model configuration:
...
- **FFT**: CMSIS-DSP's `arm_rfft_fast_f32`, built with `ARM_MATH_HELIUM` so
  it uses the M55's vector extension. Its packed output is `[0]` DC, `[1]`
  Nyquist, then `(re, im)` for bins 1 … N/2 − 1.
- **Magnitude**: `arm_cmplx_mag_f32`, `|bin| = sqrt(re² + im²)` for bins
  0–255; Nyquist is omitted.
...
**Cost.** At the default 400 MHz, the 384-frame STFT of a 2.5 s chunk takes
33 ms: FFT 6.3 M cycles, square-root compression 2.5 M, min-max 1.4 M,
magnitude 1.3 M, windowing 1.1 M, stores 0.6 M. The plain-C radix-2 FFT used
up to 1.5 took 69 ms. The output is frequency-major, so each frame's 256
magnitudes land 384 floats apart; the STFT collects eight frames before storing
them, which turns those scattered stores into runs of one cache line.

### Step 3: NPU Inference (`network.c` + LL_ATON)

```
frontend buffer → memcpy to NPU input → SCB_CleanDCache → LL_ATON_RT_Main()
         → SCB_InvalidateDCache → memcpy from NPU output → scores[NUM_CLASSES]
```

- **Input tensor** depends on the frontend: waveform samples for `raw`, a
  `[FFT_LENGTH / 2, SPEC_WIDTH]` magnitude spectrogram for `hybrid`, or a
  `[NUM_MELS, SPEC_WIDTH]` mel spectrogram for `librosa`.
- Frontend normalization learned during training is folded into batch
...
- Static working buffers (window, frame, FFT output and an 8-frame tile,
  ~12 KB): on the stack they would take most of the firmware's 16 KB.
...
- `sd_scan_audio_dir()` — enumerates up to 512 `.wav` files in a flat
  ...
  struct (~128 KB).
- The module retains result-writer helpers for experiments, but the current
  application reports results over UART and does not call them.
...
| `spec_buf` | `APP_FFT_BINS × APP_SPEC_WIDTH` × 4 bytes | float32 | 32-byte; hybrid/librosa only |
| `mel_buf` | `APP_NUM_MELS × APP_SPEC_WIDTH` × 4 bytes | float32 | 32-byte; librosa only |
...
- Frontend buffers: none (`spec_buf` and `mel_buf` are compiled out)
- Total listed application static RAM: ~378 KB

### NPU Memory (Managed by LL_ATON)

The NPU runtime allocates its own input/output buffers and activation scratch
space in `npuRAM` banks. Exact sizes depend on the frontend, class count, and
model topology; use `stedgeai analyze` for the model being deployed. Weights
are stored in external NOR flash.
...
| `APP_FFT_LENGTH` | 512 | FFT window size: a power of two from 32 to 512. |
| `APP_FFT_BINS` | 256 | Frequency bins = FFT_LENGTH / 2; Nyquist is omitted. |
...
[1/8] recording_001.wav
  ...
  [BENCH] read=71ms stft=0ms npu=13ms total=84ms
  recording_001.wav:
...
Benchmark: read=568ms stft=0ms npu=102ms total=670ms (avg read=71ms stft=0ms npu=13ms total=84ms)
```
...
all affect the result. An RTF below 1 means processing is faster than real time.
...
| CMSIS-DSP v1.16.2 (subset) | Apache-2.0 | https://github.com/ARM-software/CMSIS-DSP |
