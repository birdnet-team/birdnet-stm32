...
│    FatFs+BSP_SD fft.c (512 pt)        stedgeai output            │
└─────────────────────────────────────────────────────────────────┘
```
...
- **FFT**: Custom plain-C 512-point real FFT using the "N/2 complex FFT"
  ...
  2. Run a 256-point radix-2 DIT complex FFT (in-place, bit-reversal +
     butterfly stages).
  3. Unpack the real-FFT bins using split-radix post-processing.
  4. Twiddle factors are precomputed once on first call.
- **Magnitude**: `|bin| = sqrt(re² + im²)` for bins 0–255; Nyquist is omitted.
...
zero dependencies, and runs in ~0.1 ms per frame at 600 MHz. For a 256-frame
  spectrogram, the verified hybrid board run took about 58 ms for this stage at
...
- Use CMSIS-DSP or Helium (MVE) intrinsics to optimize the spectral paths.
